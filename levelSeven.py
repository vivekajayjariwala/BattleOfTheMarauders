...
def get_block(size):
    ...
    # choose different terrain depending on the level
...
def getUpgrade(points,power, health):
    global score, strength, life
    score = points
    strength = power
    life = health
...
def handle_move(player, objects):
    ...
    for obj in to_check:
        if obj and obj.name == "sword":
            player.make_hit()
            battleSystemDarkPage(actor,score + 20,strength,life)



def main(window):
    clock = pygame.time.Clock()
    background, bg_image = get_background("level_7.png")
    ...
    objects = [
        ...
        Block(block_size * 1, HEIGHT - block_size * 5, block_size), #Player Lands
