...
from battleSystemMushroom import *
...
def upgradePage(): 
    ...
    # Setting colors
    archer_color = (0,0,0)
    warrior_color = (0, 76, 255, 255)  # blue

    wizard_color = (59, 150, 44, 255)  # green
    ...
    archer_img = pygame.image.load('assets/upgrades/nothing.png')
    archer_img = pygame.transform.scale(archer_img, (scale_x, scale_y))

    warrior_img = pygame.image.load('assets/upgrades/power_upgrade.png')
    warrior_img = pygame.transform.scale(warrior_img, (scale_x, scale_y))

    wizard_img = pygame.image.load('assets/upgrades/health_upgrade.png')
    ...
    text_surface = font.render("  Choose Your Upgrade", True, (0, 0, 0))

    # set the text for "Next" and "Quit"
    next_text = font.render("Next", True, (0, 0, 0))

    next_rect = next_text.get_rect()
    next_rect.center = (screen_width//2, screen_height-50)
    ...
    def showCharacter(character):
        if character == "Wizard":
            nameOfUpgrade = "Health"
            text = font.render("Health", True, wizard_color)
        if character == "Archer":
            nameOfUpgrade = "Health"
            text = font.render(character, True, archer_color)
        if character == "Knight":
            nameOfUpgrade = "Power"
            text = font.render("Power", True, warrior_color)
        return text
